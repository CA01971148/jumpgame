...
export class normalScaffold extends scaffold{
    constructor(_level:number,_width:number=scaffold.defaultWidth){
        super(_level,_width)
        if(this.level===0){
            document.write('<img id="'+this.IDName+'" src="./../resource/image/scaffold/normalScaffold.jpg">')
        }else{
            this.showImg()
        }
    ...
    showImg(){
        const formerScaffoldID:string="scaffold"+String(this.level-1)
        const formerScaffold:HTMLElement=document.getElementById(formerScaffoldID)!
        formerScaffold.insertAdjacentHTML('afterend', '<img id="'+this.IDName+'" src="./../resource/image/scaffold/normalScaffold.jpg">')//足場出現
    }
}